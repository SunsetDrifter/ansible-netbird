...
class TestResourceNeedsUpdate:
    ...
    def test_name_change_is_detected(self):
        """Not reachable from ``sync_resources``, where the name IS the match
        key, so a rename is a delete-and-recreate rather than an update. Kept
        because the comparator is a general predicate and a caller that pairs
        the two some other way should still see the difference."""
    ...
    def test_surrounding_whitespace_in_a_name_is_not_a_change(self):
        """`resource_key` strips before matching, so the comparator has to as
        well -- otherwise the pair matches and then reports drift forever."""
        desired = dict(self.BASE, name=' web ')
        assert resource_needs_update(self.BASE, desired) is False
